...
o1 = Object3d([Vec3d([ 1.0, 1.0,-1.0,1]), Vec3d([-1.0, 1.0,-1.0,1]), Vec3d([-1.0, 1.0, 1.0,1]), Vec3d([ 1.0, 1.0, 1.0,1]), # Top
...
def mouseMoved(key, x, y, z):
	...
	for obj in objects:
		for i in range(len(obj.vertices)):
			if key == 3:
				obj.vertices[i] = scaleMatrix(obj.vertices[i],1.05, 1.05, 1.05)
			elif key == 4:
				obj.vertices[i] = scaleMatrix(obj.vertices[i],0.95, 0.95, 0.95)
	glutPostRedisplay()
...
def drag(x, y):
	...
	if(len(m) == 2):
		...
		for obj in objects:
			for i in range(len(obj.vertices)):
				obj.vertices[i] = rotateMatrix_x(obj.vertices[i],yVec/150)
				obj.vertices[i] = rotateMatrix_y(obj.vertices[i],xVec/150)
		m.pop(0)
		glutPostRedisplay()
